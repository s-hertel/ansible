...
def get_matching_collections(requirements, collections, force):
    ...
    for requirement in requirements:
        ...
        if os.path.isfile(b_req_name) or urlparse(req_name).scheme.lower() in ['http', 'https'] or len(req_name.split('.')) != 2:
            raise AnsibleError(message="'%s' is not a valid collection name. The format namespace.name is expected." % req_name)

        for installed_collection in collections:
...
def get_dependencies(collections, all_collections):
    """Finds all dependencies of the collections list in the list of all collections

    :param collections: A list of CollectionRequirement instances
    :param all_collections: A list of installed CollectionRequirement instances
    ...
    for collection in collections:
        for dep_name, dep_version in collection.dependencies.items():
            ...
            for installed_collection in all_collections:
...
def get_parents(dependencies, skip_parents, all_collections):
    ...
    :param all_collections: The list of installed CollectionRequirement instances
