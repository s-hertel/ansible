...
from ansible.galaxy.dependency_resolution.dataclasses import _GALAXY_YAMLS
...
class ConcreteArtifactsManager:
        ...
        try:
            ...
            raise_from(
                AnsibleError(
                    ...
                    format(to_native(collection.src), to_native(err)),
                ),
                err,
            )
    ...
    def get_artifact_path(self, collection):
        # type: (Union[Candidate, Requirement]) -> bytes
        #if not collection.is_concrete_artifact or collection.src is None:
        #    breakpoint()
        ...
        # NOTE: Requirement/Candidate types --
...
def _download_file(url, b_path, expected_hash, validate_certs, token=None):
    ...
    if expected_hash:
        display.vvvv(
            'Validating downloaded file hash {!s} with expected hash {!s}'.
...
def _normalize_galaxy_yml_manifest(
    ...
    # FIXME: possible precise type: List[Dict[str, Union[bool, str, List[str]]]]
    ...
    # FIXME: change "_build_manifest?" to accept a dict
...
def _get_meta_from_src_dir(
    ...
    galaxy_yml = next(
        (
            os.path.join(b_path, galaxy_yml)
            for galaxy_yml in _GALAXY_YAMLS
            if os.path.isfile(os.path.join(b_path, galaxy_yml))
        ),
        None,  # default
    )
    if not galaxy_yml:
        # NOTE: LookupError seems to be more semantically appropriate here:
        raise AnsibleError(
            'Failed to find the deps in {path!s}'.
...
def _get_meta_from_installed_dir(
    ...
    try:
        ...
        # NOTE: LookupError seems is more semantically appropriate here:
        raise AnsibleError(
            'Failed to find the deps in {path!s}'.
            format(path=to_native(b_path)),
        )
