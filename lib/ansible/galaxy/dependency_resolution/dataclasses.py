...
_GALAXY_YAMLS = b'galaxy.yml', b'galaxy.yaml'
...
def _is_collection_src_dir(dir_path):
    ...
    return any(
        os.path.isfile(os.path.join(b_dir_path, galaxy_yml))
...
def _find_collections_in_subdirs(dir_path):
    ...
    galaxy_yml_glob_pattern = os.path.join(
        ...
        b'*',  # collection name
        # NOTE: Python's glob.glob() does not support brace expansion so
        # NOTE: we cannot use something like `galaxy.y{a,}ml` and thus
        # NOTE: will have to just do post-filtering:
        b'galaxy.y*ml',
    )
    return (
        os.path.dirname(galaxy_yml)
        for galaxy_yml in iglob(galaxy_yml_glob_pattern)
        # NOTE: Using `str.endswith()` would hit imprecise matches
        # NOTE: forcing us to check for exact matches instead:
        if os.path.basename(galaxy_yml) in _GALAXY_YAMLS
    )
...
class _ComputedReqKindsMixin:
    ...
    def from_dir_path_as_installed(cls, dir_path, art_mgr):
        ...
        raise AnsibleError(
            ...
            format(path=dir_path, manifest_name=to_native(_MANIFEST_JSON)),
        )
    ...
    def __repr__(self):
        return (
            '<{self!s} of type {self.type!r} from {src!s}>'.
            format(self=self, src=self.src or 'Galaxy')
        )
