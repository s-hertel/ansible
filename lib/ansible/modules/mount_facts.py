...
options:
  ...
  sources:
    description:
      ...
      - The C(mount_points) return value contains the first definition found for a mount_point.
...
def gen_mounts_by_source(module: AnsibleModule):
    ...
    for source in sources:
        ...
        mounts = gen_mounts_by_file([source])
        yield from mounts
...
def get_argument_spec():
    ...
    return dict(
        ...
        show_aggregate_mounts=dict(default=True, type="bool"),
    )
...
def main():
    ...
    if not module.params["show_aggregate_mounts"]:
        aggregate_mounts = []
