...
from ansible.galaxy.collection import build_collection, install_collections, publish_collection, \
    validate_collection_name, verify_collections
...
class GalaxyCLI(CLI):
    ...
    def add_verify_options(self, parser, parents=None):
        ...
        verify_parser = parser.add_parser('verify', parents=parents, help='Compare checksums one the galaxy server with the installed copy')
        verify_parser.set_defaults(func=self.execute_verify)

        path_help = 'The path to the directory containing your collections.'
        name_help = 'The collection(s) name or path/url to a tar.gz collection artifact. This is mutually exclusive with --requirements-file.'
        req_help = 'A file containing a list of collections to be verified.'
        no_deps_help = "Don't verify collections listed as dependencies"
        ...
        verify_parser.add_argument('-p', '--collections-path', dest='collections_path', default=C.COLLECTIONS_PATHS[0], help=path_help)
        verify_parser.add_argument('args', metavar='{0}_name'.format(galaxy_type), nargs='*', help=name_help)
        verify_parser.add_argument('-i', '--ignore-errors', dest='ignore_errors', action='store_true', default=False, help=ignore_errors_help)
        verify_parser.add_argument('-r', '--requirements-file', dest='requirements', help=req_help)
        verify_parser.add_argument('-n', '--no-deps', dest='no_deps', action='store_true', default=False, help=no_deps_help)
...
############################
    ...
    def execute_verify(self):
        ...
        verify_collections(requirements, search_path, self.api_servers, (not ignore_certs), ignore_errors, no_deps)
        return 0
