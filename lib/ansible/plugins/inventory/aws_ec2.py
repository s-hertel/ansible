...
group_by:
  ...
  # create a group for each instance state found
  - state
...
# This gives an array of keys to get from the filter name to the value
# returned by boto3's EC2 describe_instances method.
ec2_filter_to_boto_attr = {
    ...
    'dns-name':
        ['PublicDnsName'],
    'group-id':
        ['Reservations', 'Groups', 'GroupId'],
    'group-name':
        ['Reservations', 'Groups', 'GroupName'],
    'host-id':
        ['Placement', 'HostId'],
    'hypervisor':
        ['Hypervisor'],
    ...
    'network-interface.addresses.primary':
        ['NetworkInterfaces', 'PrivateIpAddresses', 'Primary'],
    'network-interface.addresses.association.public-ip':
        ['NetworkInterfaces', 'PrivateIpAddresses', 'Association', 'PublicIp'],
    'network-interface.addresses.association.ip-owner-id':
        ['NetworkInterfaces', 'PrivateIpAddresses', 'Association', 'IpOwnerId'],
    'network-interface.association.public-ip':
        ['NetworkInterfaces', 'Association', 'PublicIp'],
    'network-interface.association.ip-owner-id':
        ['NetworkInterfaces', 'Association', 'IpOwnerId'],
    'network-interface.association.allocation-id':
        ['ElasticGpuAssociations', 'ElasticGpuId'],
    'network-interface.association.association-id':
        ['ElasticGpuAssociations', 'ElasticGpuAssociationId'],
    ...
    'network-interface.requester-id': ['RequesterId'],
    'network-interface.requester-managed': [], # FIXME
...
class InventoryModule(BaseInventoryPlugin):
    ...
    def _get_group_by_value(self, group, instance):
        ...
        if 'tag' in group:
            tags = boto3_tag_list_to_ansible_dict(instance_value.get('Tags', []))
        ...
        return self._get_boto_attr_chain(group, instance_value)
    ...
    def _get_boto_attr_chain(self, filter_name, instance):
        ...
        '''
        allowed_filters = sorted(list(ec2_filter_to_boto_attr.keys()))
        if filter_name not in allowed_filters:
            raise AnsibleError("Illegal filter provided. %s must be one of %s." % (filter_name,
                                                                                   allowed_filters))
        boto_attr_list = ec2_filter_to_boto_attr[filter_name]

        instance_value = instance
        for attribute in boto_attr_list:
            instance_value = instance_value[attribute]
        ...
        for region in regions:
            try:
                ...
                if self.boto_profile:
                    ...
                    raise AnsibleError("Insufficient credentials provided.")
    ...
    def _get_instances_by_region(self, regions, filters, strict_permissions):
        ...
        for connection, region in self._boto3_conn(regions):
            try:
                instances = connection.describe_instances(Filters=filters)
            ...
            all_instances.extend(self._format_instance_data(region, instances.get('Reservations')[0]))
    ...
    def _get_hostname(self, instance, hostnames):
        ...
        for preference in hostnames:
            try:
                hostname = self._get_boto_attr_chain(preference, instance.instance_data)
        ...
        for group_n in groups:
            if group_n == 'aws_ec2':
                ...
                for group_v in groups[group_n]:
                    if group_n == 'tag:':
                        groupname = to_safe_group_name(self.group_prefix + group_n + group_v)
                    else:
                        groupname = to_safe_group_name(self.group_prefix + group_n + '_' + group_v)
