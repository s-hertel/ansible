            ...
            if _evaluate_conditional(target_host):
                for host in self._inventory.get_hosts(iterator._play.hosts):
                    ...
                    if failed and host.name in self._tqm._stats.rescued and self._tqm._stats.rescued[host.name]:
                        iterator._host_states[host.name].restore_run_state()
                    elif failed:
                        iterator._host_states[host.name].restore_properties()
                    # propagate all failed and unreachable hosts as they can be an arbitrary number of tasks behind
